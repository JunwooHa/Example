...
/*
 	...
 	@Before 	: 지정 포인트 찾아 실행되기 전에 수행
 	@After 		: 지정 포인트 찾아 실행된 후 수행
 	
 	PointCut		: AOP가 실행될 위치를 의미
 	1. execution	: 실행할 위치를 지정한다
 	2. *			: 반환형 자리 (*는 모든 타입을 의미)
 	3. com.itbank.service : 패키지 명
 	4. BoardService	: 클래스명
 	5. *			: 매서드명
...
public class TestAOP {
	...
	@Before("execution(* com.itbank.service.BoardService.*(com.itbank.model.dto.BoardDTO))")
	public void change(JoinPoint jp) {
		
		BoardDTO input = (BoardDTO) jp.getArgs()[0];
		
		// input.setTitle(input.getTitle() + "※AOP 개입~");
		// - 매번 게시글 제목에 추가 되기 때문에 제거 (= 디버그용)
