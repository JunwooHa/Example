...
public class BoardController {
	...
	public ModelAndView view(int idx) {
		...
		bs.CountupBoard(idx);								
		List<ReplyDTO> replys = rs.getReply(idx);
		
		mav.addObject("row", bs.getBoard(idx));
		mav.addObject("rps", replys);
	...
	public ModelAndView view(ReplyDTO input) {
		...
		mav.addObject("row", rs.insert(input));
		mav.addObject("message", "댓글 추가 ");
	...
	public ModelAndView ReplyDelete(@PathVariable int idx) {
		ModelAndView mav = new ModelAndView("msg");
		
		mav.addObject("row", rs.delete(idx));
		mav.addObject("message", "댓글 삭제");
